...
special_words=["SOS","save_me","help_me","help","save"] # Set of special words to check in encoded text the spy will send
...
def add_friend(spy_name): #method to add a friend
    ...
    if f_rating<spy_detail[spy_name]["rating"]: #checks rating
        ...
        return(0)
    spy_detail[spy_name]["friends"].update({f_name:{"f_age":f_age,"f_rating":f_rating,"chat":{}}}) #updates and adds friend to spy_details dictionary
...
def send_a_message(spy_name): #method to send an encryted message
    ...
    if position=="null":
        ...
        count=count+1
        spy_detail[spy_name]["friends"][f_name]["chat"].update({count:{"text":text, "time":date,"boolean":True}}) #stores in chat dictionary under spy_detail dictionary
...
def chat_read(spy_name):
    ...
    if position=="null":
        return()
    f_name=spy_detail[spy_name]["friends"].keys()[position] #gets the name of the friend from the position
